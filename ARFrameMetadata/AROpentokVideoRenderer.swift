...
class AROpentokVideoRenderer : NSObject, OTVideoRender {
    ...
    fileprivate func createTextures(withFormat format: OTVideoFormat) {
        ...
        //view.currentDrawable?.texture = rgbTexture
    ...
    func renderVideoFrame(_ frame: OTVideoFrame) {
        ...
        if let metadata = frame.metadata {
            let values = metadata.toArray(type: Float.self)
            print("\(values)")
        }
